...
import { Saveable, SaveableSource, SelectionService } from "@MagicIdea/core";
import { BaseWidget } from "@MagicIdea/core/widgets";
import { Navigatable } from "@MagicIdea/core";
import URI from "@MagicIdea/core/common/uri";
import { Message } from "@lumino/messaging";
import { TextEditor } from "./text-editor";

import { Widget, TabBar, DockPanel, Title } from "@lumino/widgets";
...
export class EditorWidget extends BaseWidget implements SaveableSource, Navigatable {
  ...
  protected checkForTabbarChange(): void {
    ...
    if (parent instanceof DockPanel) {
      const newTabbar = find(
        parent.tabBars(),
        (tabbar) => !!tabbar.titles.find((title) => title === this.title)
      );
      if (this.currentTabbar !== newTabbar) {
        this.toDisposeOnTabbarChange.dispose();
        const listener = () => this.checkForTabbarChange();
        parent.layoutModified.connect(listener);
        this.toDisposeOnTabbarChange.push(
          Disposable.create(() => parent.layoutModified.disconnect(listener))
        );
  ...
  protected handleTabBarChange(
    oldTabBar?: TabBar<Widget>,
    newTabBar?: TabBar<Widget>
  ): void {
    const ownSaveable = Saveable.get(this);
    const competingEditors =
      ownSaveable &&
      newTabBar?.titles.filter(
        (title) =>
          title !== this.title &&
          title.owner instanceof EditorWidget &&
          title.owner.editor.uri.isEqual(
            this.editor.uri
          ) &&
          Saveable.get(title.owner) === ownSaveable
      );
    competingEditors?.forEach((title: Title<Widget>) => title.owner.close());
  }
  ...
  storeState(): object | undefined {
    return this.getResourceUri()?.scheme === UNTITLED_SCHEME
      ? undefined
      : this.editor.storeViewState();
  }
