...
import { RecursivePartial, Emitter, IEvent as Event } from "@MagicIdea/core/common";
...
export class EditorManager extends NavigatableWidgetOpenHandler<EditorWidget> {
  ...
  readonly onActiveEditorChanged: Event<EditorWidget | undefined> =
    this.onActiveEditorChangedEmitter.event;
  ...
  readonly onCurrentEditorChanged: Event<EditorWidget | undefined> =
    this.onCurrentEditorChangedEmitter.event;
  ...
  private fireCurrentEditorChangedEvent(current: EditorWidget | undefined): void{
    this.onCurrentEditorChangedEmitter.fire(current);
  }
  ...
  openToSide(uri: URI, options?: EditorOpenerOptions): Promise<EditorWidget> {
    ...
    const splitOptions: EditorOpenerOptions = {
  ...
  override async open(
    ...
    try {
      if (options?.counter === undefined) {
        const insertionOptions = this.shell.getInsertionOptions(
          options?.widgetOptions
        );
        // Definitely creating a new tabbar - no widget can match.
        if (insertionOptions.addOptions.mode?.startsWith("split")) {
          return await super.open(uri, {
            counter: this.createCounterForUri(uri),
            ...options,
          });
        }
        // Check the target tabbar for an existing widget.
        const tabbar =
          insertionOptions.addOptions.ref &&
          this.shell.getTabBarFor(insertionOptions.addOptions.ref);
        if (tabbar) {
          const currentUri = uri.toString();
          for (const title of tabbar.titles) {
            if (title.owner instanceof EditorWidget) {
              const { uri: otherWidgetUri, id } = this.extractIdFromWidget(
                title.owner
              );
  ...
  protected override async doOpen(widget: EditorWidget, uri: URI, options?: EditorOpenerOptions): Promise<void> {
    ...
    await this.revealSelection(widget, options);
  }

  protected async revealSelection(widget: EditorWidget, options?: EditorOpenerOptions): Promise<void> {
    let inputSelection = options?.selection;
    if (!inputSelection) {
        inputSelection = await this.resolveSelection(options ?? {});
    }

    if (inputSelection) {
        const editor = widget.editor;
        const selection = this.getSelection(inputSelection);
        if(selection){
  ...
  protected async resolveSelection(options: EditorOpenerOptions): Promise<RecursivePartial<Range> | undefined> {
  ...
  protected getCounterForUri(uri: URI): number | undefined {
    ...
    const counterOfMostRecentlyVisibleEditor = this.recentlyVisibleIds
      .find((id) => id.startsWith(idWithoutCounter))
      ?.slice(idWithoutCounter.length + 1);
    return counterOfMostRecentlyVisibleEditor === undefined
      ? undefined
      : parseInt(counterOfMostRecentlyVisibleEditor);
  }
  ...
  protected setCurrentEditor(current: EditorWidget | undefined): void {
    if (this._currentEditor !== current) {
      ...
      this.fireCurrentEditorChangedEvent(this._currentEditor);
    }
  }
  protected updateCurrentEditor(): void {
    ...
    if (widget instanceof EditorWidget) {
      ...
      !this._currentEditor ||
      !this._currentEditor.isVisible ||
      this.currentEditor !== this.recentlyVisible
    ) {
