...
export namespace ApplicationShellLayoutMigrationError {
  ...
  export function create(
    ...
    return Object.assign(
      new Error(
        `Could not migrate layout to version ${applicationShellLayoutVersion}.` +
          (message ? "\n" + message : ""),
      ),
      { code },
    );
  ...
  ): error is ApplicationShellLayoutMigrationError {
    return !!error && "code" in error && error["code"] === code;
  }
}

export const ApplicationShellLayoutMigration = Symbol(
...
export interface ApplicationShellLayoutMigration {
   ...
   */
  onWillInflateLayout?(
    context: ApplicationShellLayoutMigrationContext,
  ): MaybePromise<void>;
  ...
  onWillInflateWidget?(
    ...
    context: ApplicationShellLayoutMigrationContext,
  ): MaybePromise<WidgetDescription | undefined>;
}
...
export const RESET_LAYOUT = { id: "reset.layout", label: "重置布局" };

@injectable()
export class ShellLayoutRestorer implements CommandContribution {
  private logger = getLogger("shell-layout-restorer");
  ...
  constructor(
    ...
    @inject(MagicApiServerService) protected readonly magicApiServerService: MagicApiServerService
  ) {
    this.magicApiServerService.onDidChangeProject((data) => {
      this.updatePrefix(data?.id);
    });
  }

  registerCommands(commands: CommandRegistry): void {
    commands.addCommand(RESET_LAYOUT.id, {
      execute: async () => this.resetLayout(),
    });
  }
    
  private updatePrefix(projectId: string | undefined): void {
    this.prefix = projectId || '_global_';
  }
  ...
  protected async resetLayout(): Promise<void> {
    const fullKey = this.prefixWorkspaceURI(this.storageKey);
    this.logger.info(">>> Resetting layout...", fullKey);
    this.shouldStoreLayout = false;
    this.storageService.setData(fullKey, undefined);
  ...
  storeLayout(app: FrontendApplication): void {
    ...
    if (this.shouldStoreLayout) {
      try {
        this.logger.info(">>> Storing the layout...", fullKey);
        const layoutData = app.shell.getLayoutData();
        const serializedLayoutData = this.deflate(layoutData);
        this.storageService.setData(fullKey, serializedLayoutData);
  ...
  async restoreLayout(app: FrontendApplication): Promise<boolean> {
    const fullKey = this.prefixWorkspaceURI(this.storageKey);
    this.logger.info(">>> Restoring the layout state...", fullKey);
    const serializedLayoutData = await this.storageService.getData<string>(fullKey);
    ...
    this.transformations
      ...
      .forEach((transformation) =>
        transformation.transformLayoutOnRestore(layoutData),
      );
   ...
   */
  protected async inflate(
    layoutData: string,
  ): Promise<ApplicationShellLayout.LayoutData> {
    const parseContext = new ShellLayoutRestorer.ParseContext();
    const layout = this.parse<ApplicationShellLayout.LayoutData>(
      layoutData,
      parseContext,
    );
    ...
    if (layoutVersion !== applicationShellLayoutVersion) {
      if (layoutVersion < applicationShellLayoutVersion) {
        console.warn(
          `Layout version ${layoutVersion} is behind current layout version ${applicationShellLayoutVersion}, trying to migrate...`,
        );
      } else {
        console.warn(
          `Layout version ${layoutVersion} is ahead current layout version ${applicationShellLayoutVersion}, trying to load anyway...`,
        );
      }
      console.info(
        `Please use '${RESET_LAYOUT.label}' command if the layout looks bogus.`,
      );
    }
    ...
    const migrations = this.migrations
      ...
      .filter(
        (m) =>
          ...
          m.layoutVersion <= applicationShellLayoutVersion,
      )
      .sort((m, m2) => m.layoutVersion - m2.layoutVersion);
    if (migrations.length) {
      console.info(
        `Found ${migrations.length} migrations from layout version ${layoutVersion} to version ${applicationShellLayoutVersion}, migrating...`,
      );
    }
      ...
      if (this.isWidgetsProperty(property)) {
        ...
        for (const p in value) {
          if (this.isWidgetProperty(p)) {
            parseContext.push(async (context) => {
              copy[p] = await this.convertToWidget(
                value[p] as WidgetDescription,
                context,
              );
            });
          } else {
  ...
  protected async fireWillInflateWidget(
    desc: WidgetDescription,
    context: ShellLayoutRestorer.InflateContext,
      ...
      if (migration.onWillInflateWidget) {
        ...
        if (migrated) {
          if (isObject(migrated.innerWidgetState)) {
            ...
            migrated.innerWidgetState = JSON.stringify(
              migrated.innerWidgetState,
            );
          }
          desc = migrated;
        }
  ...
  protected async convertToWidget(
    ...
    try {
      ...
      const widget = await this.widgetManager.getOrCreateWidget(
        desc.constructionOptions.factoryId,
        desc.constructionOptions.options,
      );
      if (StatefulWidget.is(widget) && desc.innerWidgetState !== undefined) {
        try {
          ...
          this.logger.warn(
            `Couldn't restore widget state for ${widget.id}. Error: ${e} `,
          );
        }
      }
      ...
      this.logger.warn(
        `Couldn't restore widget for ${desc.constructionOptions.factoryId}. Error: ${e} `,
      );
      return undefined;
    }
  }
