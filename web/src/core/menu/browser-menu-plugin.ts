...
import { ApplicationShellLayout as ApplicationShell } from '../shell';
import { ElementExt } from '@lumino/domutils';
import { CommandMenu, CompoundMenuNode, MAIN_MENU_BAR, MenuNode, MenuPath, RenderedMenuNode, Submenu, MenuModelRegistry } from '../common/menu';
...
export class BrowserMainMenuFactory implements MenuWidgetFactory {
    ...
    protected readonly corePreferences: PreferenceService;
    ...
    createMenuBar(): MenuBarWidget {
        ...
        menuBar.id = 'magic-idea:menubar';
        this.corePreferences.ready.then(() => {
            this.showMenuBar(menuBar);
        });
        const disposable = new DisposableCollection(
            this.corePreferences.onDidPreferenceChanged(change => {
                if (change.key === 'window.menuBarVisibility' && typeof change.newValue === 'string') {
