...
class TokenStream {
  ...
  match(tokenOrText, consume, ignoreCase) {
    ...
    if (Array.isArray(tokenOrText)) {
      ...
      if (
        this.tokens[this.index].getText() === tokenOrText ||
        (ignoreCase === true && this.tokens[this.index].getText().toLowerCase() === tokenOrText.toLowerCase())
      ) {
        match = true
      }
    } else {
    ...
    if (this.match(text, true, ignoreCase)) {
      ...
      if (!this.hasMore()) {
        ...
        throw new ParseException(
          "Expected '" + this.textToString(text) + "', but got '" + token.getText() + "'",
          token.getSpan()
        )
      }
    }
