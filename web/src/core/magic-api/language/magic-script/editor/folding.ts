...
let computeIndentLevel = function (line: any, tabSize: any) {
  var indent = 0
  var i = 0
  var len = line.length
  while (i < len) {
...
const FoldingRangeProvider = {
  provideFoldingRanges: (model: any, context: any) => {
    ...
    for (let line_1 = model.getLineCount(); line_1 > 0; line_1--) {
      ...
      let previous: any = previousRegions[previousRegions.length - 1]
      if (indent === -1) {
